...
async def handle_callback(update: Update, context):
    ...
    try:
        if action == "deposit":
            ...
            if str(chat_id) == str(ADMIN_ID):
                await query.message.reply_text("Admin features:\n- Database: Not connected (demo mode)\n- Web panel: Run Flask app separately")
            else:
                await query.message.reply_text("Access denied. Not an admin.")
    except Exception as e:
...
def main():
    ...
    app.add_handler(MessageHandler(filters.COMMAND, lambda update, context: update.message.reply_text("Unknown command. Use /start to begin.")))
