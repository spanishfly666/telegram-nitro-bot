...
def send_document(chat_id,filename):
    ...
    with open(path,'rb') as doc:
        files={'document':doc}; data={'chat_id':chat_id}
...
def webhook():
    ...
    update_id=data.get('update_id'); uid=None
    ...
    # Messages
    if 'message' in data:
        msg=data['message']; chat_id=msg['from']['id']; text=msg.get('text','').strip()
        if deposit_requests.get(chat_id)=='await_amount':
            try:
                usd=float(text); order_id=f'{chat_id}_{int(time.time())}'
    ...
    if 'callback_query' in data:
        cb=data['callback_query']; chat_id=cb['from']['id']; action=cb['data']
        ...
        if action=='deposit':
            ...
            deposit_requests[chat_id]='await_amount'; send_message(chat_id,'Enter USD amount to deposit:')
            ...
            bal=get_balance(chat_id); send_message(chat_id,f'Your balance: {bal:.2f} credits')
            ...
            pid=int(action.split('_',1)[1]); bal=get_balance(chat_id)
            conn=sqlite3.connect(db_path); c=conn.cursor();
            c.execute('SELECT name,filename,price FROM products WHERE id=?',(pid,)); row=c.fetchone(); conn.close()
            if not row:
