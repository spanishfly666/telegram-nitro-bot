...
# Custom ModelView for Users: add credit action
...
@app.route('/webhook', methods=['POST'])
 def webhook():
    ...
    conn=sqlite3.connect(DB_PATH)
    c=conn.cursor()
    c.execute('INSERT OR IGNORE INTO messages (update_id, user_id, raw_data) VALUES (?,?,?)',(str(update_id),uid,json.dumps(data)))
    ...
    if status in ('confirmed','partially_paid'):
        ...
        # Convert to credits
        est=requests.get('https://api.nowpayments.io/v1/estimate',params={'source_currency':'BTC','target_currency':'USD','source_amount':btc_amt},headers={'x-api-key':NOWPAYMENTS_API_KEY}).json()
