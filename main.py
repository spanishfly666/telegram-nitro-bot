...
class BulkUploadView(BaseView):
    ...
    def index(self):
        ...
        return self.render("admin/bulk_upload.html", message=message)
...
def send_document(chat_id, filename):
    with open(os.path.join(FILES_DIR, filename), "rb") as f:
        ...
        requests.post(
            f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendDocument", files=files, data=data
        )
...
def webhook():
    ...
    if "message" in data:
        ...
        if text == "/start":
            ...
            if chat_id in (OWNER_ID, ADMIN_ID):
    ...
    if "callback_query" in data:
        ...
        if action == "deposit":
            ...
            send_message(chat_id, "Please contact the admin for manual deposits.")
        elif action == "admin":
