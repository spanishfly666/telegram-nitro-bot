...
# --- Create tables (won't crash if they already exist) ---
with app.app_context():
    try:
        db.create_all()
    except Exception as e:
        app.logger.warning(f"Skipping create_all: {e}")
    ...
    def index(self):
        ...
        if request.method == 'POST':
            ...
            for line in lines:
                parts = line.split('|')
                if len(parts) == 3:
                    name, fn, price = [p.strip() for p in parts]
                    prod = Product(name=name, filename=fn, price=float(price))
                    db.session.add(prod)
                    count += 1
            db.session.commit()
...
def get_balance(user_id):
    ...
    c = conn.cursor()
    c.execute(
        'INSERT OR IGNORE INTO users (id, balance, role) VALUES (?, 0.0, "user")',
        (user_id,)
    )
...
def webhook():
    ...
    if status in ('confirmed', 'partially_paid'):
        ...
        send_message(user_id, f'Your deposit has been credited as {credits:.2f} credits.')
        return '', 200
        ...
        if action == 'deposit':
            ...
            if not row:
                ...
                if bal < pr:
                    ...
                    sale = Sale(user_id=chat_id, product_id=pid)
                    db.session.add(sale)
                    db.session.commit()
